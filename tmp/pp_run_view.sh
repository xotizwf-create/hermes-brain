...
sshpass -p "$PW" scp -o StrictHostKeyChecking=accept-new /tmp/pp_view_real.py "$RUSER@$IP:/tmp/pp_view_real.py" || exit 3
sshpass -p "$PW" ssh -o StrictHostKeyChecking=accept-new "$RUSER@$IP" '/var/www/prostye-postavki/app/.venv/bin/python /tmp/pp_view_real.py'
