...
sshpass -p "$PW" scp -o StrictHostKeyChecking=accept-new /tmp/pp_swap.sh "$RUSER@$IP:/tmp/pp_swap.sh" || exit 3
sshpass -p "$PW" ssh -o StrictHostKeyChecking=accept-new "$RUSER@$IP" 'bash /tmp/pp_swap.sh'
