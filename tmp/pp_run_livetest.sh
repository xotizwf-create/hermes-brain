...
sshpass -p "$PW" scp -o StrictHostKeyChecking=accept-new /tmp/pp_livetest.py "$RUSER@$IP:/tmp/pp_livetest.py" || exit 3
sshpass -p "$PW" ssh -o StrictHostKeyChecking=accept-new "$RUSER@$IP" '/var/www/prostye-postavki/app/.venv/bin/python /tmp/pp_livetest.py'
