...
### Albery Agent Center / per-agent knowledge registry

When the owner asks to add or change a durable behavior for an Albery subagent, do it through Albery's GitHub-backed Agent Center registry, not as a local-only Hermes note. Create/update `agent_knowledge/skills/<skill>/SKILL.md`, connect it in `agent_knowledge/agents/<slug>.yaml` using the id form `skill:<skill>`, and verify with `agent_center.agent_selected_knowledge(agent)` before reporting success. Do not infer the agent slug from the visible name: historical slugs can be misleading (for example the visible `Агент-юрист` may still use `agent-sklad`). Detailed runbook: `references/albery-agent-center-skill-registry.md`.

### Albery Google Sheets / Docs quality guards
...
When the owner reports that the agent cannot read a private Google Sheet/Doc despite the file being shared with the agent account, inspect the URL-fetch path separately from the create/write/format tools. A fetcher that rewrites Google links to public CSV/TXT export and uses anonymous HTTP will still fail on private files. Reuse the existing OAuth credentials: Sheets API for spreadsheets (respecting `gid`) and Drive API `text/plain` export for Google Docs, falling back to the ordinary HTTP path only for non-Google URLs. After a code fix, also update the MCP/tool description the LLM sees, clear stale “Google 401” registry/source-data notes that are no longer true, restart the app/gateway that serves tool schemas, and verify with private test Sheet/Doc markers before reporting success. Detailed runbook: `references/google-oauth-private-file-fetch.md`.
...
- `references/remote-hermes-codex-device-auth.md` — remote `openai-codex` device-code re-auth when SSH/interactive output hides the normal login prompt
