...
- **Albery Bitrix/Hermes model confusion, `Broken pipe`/empty AI replies, or slow simple Bitrix-agent requests hitting 300–600s timeouts:** use `references/albery-bitrix-hermes-model-routing.md`; check both the global Hermes config and app-level Albery `.env` overrides such as `B24_TESTBOT_MODEL`, then separate underlying data-source speed from the full Hermes wrapper path before blaming MCP/DB.
