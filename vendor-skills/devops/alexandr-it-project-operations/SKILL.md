...
- **«Простые поставки» MCP prompt navigation / instruction search:** when adding or repairing built-in MCP instructions, prompt metadata, `get_mcp_prompt`, prompt lists, or natural-language instruction search, follow `references/prostye-postavki-mcp-prompt-navigation.md`: keep prompt bodies canonical in the project repo, expose read-only `list_mcp_prompt_topics`/`search_mcp_prompts`, verify with real owner wording and live MCP `tools/list`, then document the map in Hermes Brain.
...
- **Albery Bitrix/Hermes model confusion, `Broken pipe`/empty AI replies, slow simple Bitrix-agent requests, or AI-lawyer contract generation hitting 300–600s timeouts:** use `references/albery-bitrix-hermes-model-routing.md`; for «Агент-юрист» document/contract failures also use `references/albery-bitrix-legal-document-timeouts.md`. Check both the global Hermes config and app-level Albery `.env` overrides such as `B24_TESTBOT_MODEL`, then separate underlying data-source speed, prompt/tool bloat, background-job needs, and the full Hermes wrapper path before blaming MCP/DB.
