...
- `send_bitrix_message(recipient_bitrix_user_id?, recipient_name?, message_text, confirm)` —
  **добавлен 28.05.2026**: отправляет одно личное сообщение в Bitrix любому
  ...
  45 инструментов).
- `get_employee_absences(employee_name?, bitrix_user_id?, date_from?, date_to?)` —
  **добавлен 2026-06-22**: видит, кто **в отпуске/отсутствует** по Bitrix «График
  отсутствий». Читает **портал бота b24-0xrp3s** (туда компания переезжает и там
  включён scope `calendar`), через `calendar.accessibility.get` — берёт только записи
  с `ACCESSIBILITY='absent'` (отпуск/больничный/командировка), обычные «busy»-встречи
  игнорирует. Резолв: `bitrix_user_id` ИЛИ нечёткий `employee_name` (по активным
  пользователям b24, `_person_names_match`; неоднозначность → список кандидатов). Без
  обоих — возвращает всех отсутствующих в периоде (по умолчанию сегодня). Для проверки
  на конкретную дату — `date_from=date_to` (напр. дедлайн задачи). Отдаёт `on_vacation`
  + периоды. **Только полный доступ** (НЕ в `FAQ_TOOL_NAMES` — приватные данные).
  Хелперы: `_b24_webhook_call`, `_b24_active_users` (кэш 1ч), `_b24_absence_periods`.
  Проверено вживую (создал absent-событие → tool вернул `on_vacation=true` → удалил).
  **Важно (портальная развязка):** зум-задачи сейчас создаются на **damaska**
  (`BITRIX_WEBHOOK_BASE`), а отпуска — на **b24-0xrp3s** → id сотрудников НЕ совпадают.
  Поэтому авто-пропуск отпускников в `dispatch_zoom_operational_tasks` пока **НЕ включён**:
  он заработает, когда задачи начнут ставиться на тот же портал, что и отпуска (после
  переезда на b24-0xrp3s). Правило для зум-флоу (когда активируем): перед постановкой —
...
### Дедлайны по рабочим часам: зум-задачи и рекомендации (2026-07-09)

Правила владельца (рабочий день 9:00–18:00 МСК, пн–пт), модуль **`business_hours.py`**
(Albery `98a8ef1` + фронт `23d3146`; всё в env: `ZOOM_LEAD_MIN_GAP_HOURS`,
`ZOOM_LEAD_NEXT_DAY_HOUR`, `RECOMMENDATIONS_DEADLINE_HOUR`, `WORKDAY_END_HOUR`):

- **Зум-карточка лиду** (`zoom_dispatch_deadline` в zoom.py): дедлайн = **18:00 МСК в день
  ОТПРАВКИ**; если до 18:00 осталось меньше 3 часов или выходной — **следующий рабочий день
  11:00**. Дата созвона сознательно игнорируется (раньше было жёстко 19:00 даты созвона — при
  отправке старого созвона дедлайн оказывался в прошлом). Фронт-превью в App.tsx
  (`zoomCallDispatchDeadline`) зеркалит то же правило.
- **Рекомендации owner-daily** (`owner_recommendations_task_deadline` в app.py): задачи-«Рекомендации
  DD.MM» получают дедлайн **12:00 следующего РАБОЧЕГО дня** (пятничный отчёт → понедельник 12:00;
  было 10:00 следующего календарного, включая субботу). ВАЖНО: рекомендации уже давно создаются
  ЗАДАЧАМИ (`tasks.task.add`, `ALLOW_CHANGE_DEADLINE=N`), не личными сообщениями — старые записи в
  этом доке про `im.message.add` устарели.
- Живые задачи агентов (`create_bitrix_task`) по решению владельца НЕ трогали — никаких минимумов.
- Деплой без убийства живых ходов: рестарт только при `bitrix_inflight_turns`=0 и
  0 running-автоматизаций (скрипт-паттерн safe_restart). Тесты `tests/unit/test_business_hours.py`.
...
- `GET/POST /applet/<token>` (`app.py`, в `AUTH_EXEMPT_PREFIXES`, CORS `*`): GET → `{values:[[...]]}`,
  POST `{values:[...]}` → добавляет строку (`values().append`). Подпись токена — HMAC от id таблицы
  (+лист) на серверном ключе приложения; доступ только к ОДНОЙ таблице, не угадывается.
