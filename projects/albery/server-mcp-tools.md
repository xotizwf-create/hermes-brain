...
Фикс: `B24_TESTBOT_HERMES_TIMEOUT=300` в `/var/www/albery/.env` (есть typing-keepalive + фоновый
поток, длинный ход безопасен) + `format_google_sheet` возвращает лёгкий `replies_count` вместо
большого `replies` (меньше рост контекста → быстрее каждый шаг). Коммит `a928f10`. Если сложные
сборки снова упрутся в лимит — следующий шаг это композитный инструмент (вся таблица за ОДИН
MCP-вызов, чтобы убрать межшаговую латентность модели).
