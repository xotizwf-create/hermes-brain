...
- **Policy-routing** (`/root/vpn_apply.sh`, запускается как `PostUp` туннеля):
  ...
  - точечные исключения для ресурсов, которые блокируют VPN-выход, идут через
    `main`/`eth0`; сейчас так закреплён `hh.ru` (`94.124.200.0/24`) — VPN при этом
    не отключается, меняется только маршрут к этому назначению;
...
ip rule show                        # должны быть правила 900/901/902/950/1000/1001
...
### Если внешний ресурс не открывается

Не считать сразу, что «сайт лежит». На этом сервере частая причина — страна
исходящего IP: один сервис может требовать VPN, другой наоборот режет VPN и
открывается только с российского IP.

Порядок проверки:

1. Проверить код ошибки обычным маршрутом и внешний IP по умолчанию.
2. Проверить маршрут к домену/IP: `ip route get <ip>`.
3. Проверить тот же ресурс через прямой WAN-интерфейс: `curl --interface eth0 ...`.
4. Если через VPN ошибка, а через `eth0` ресурс открывается — не отключать VPN.
   Добавить постоянное адресное исключение в `/root/vpn_apply.sh` в
   `VPN_BYPASS_DESTS`, а в `/root/vpn_rollback.sh` добавить снятие соответствующего
   правила по priority.
5. Применить `/root/vpn_apply.sh` и проверить три вещи: правило не дублируется,
   проблемный ресурс открывается, общий внешний IP по умолчанию всё ещё VPN.

Кейс 2026-06-02: `hh.ru` отдавал блокировку через VPN-выход `95.85.243.43`, но
открывался через российский IP `217.198.12.236`. Постоянное исключение добавлено
для `94.124.200.0/24` с priority `950` через таблицу `main`.
