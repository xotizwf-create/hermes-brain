...
> ⛔ **27.07.2026 отчётность переехала с Telegram-бота Hermes на агента Албери** (владелец).
> Токен бота был отозван, и вместе с ним замолчала вся отчётность. Теперь четыре задания —
> «Ежедневный отчёт собственнику», «Недельный отчёт собственнику», «Дайджест по
> руководителям», «Разбор ошибок агента за неделю» — это **родные автоматизации Албери**
> (`agent_automations`, агент `main`, доставка в Битрикс владельцу id 16). Расписания и
> промпты те же, мозг тот же; в Hermes эти задания стоят на паузе. Часовой сторож здоровья
> переехал отдельно: `/usr/local/bin/albery-selfcheck-watch` (cron `7 * * * *`) → доклад в
> Битрикс. В Hermes намеренно остался только `zoom-to-tasks` — это движок разбора созвонов,
> а не рассылка.
>
> **Грабли переноса:** правило тишины. У Hermes оно звучало «верни ПУСТОЙ ответ», а движок
> автоматизаций Албери на пустой ответ отвечает ошибкой «пустой ответ мозга» — штатная
> тишина превратилась бы в сбой. В Албери тишина — это ровно слово `SILENT`.
>
> Описание ниже — как это работало ДО переноса. Задача Битрикса 2198.
