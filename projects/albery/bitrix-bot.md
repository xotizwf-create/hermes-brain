...
`B24_DISCLAIMER` — сноска под КАЖДЫМ ответом бота (default `[i]Ответы Албери AI могут быть неточными.
Проверяйте важную информацию.[/i]`; добавляется в `_b24_app_reply`; пустая строка = отключить; если
портал не рендерит BBCode `[i]` — поменять на обычный текст через эту переменную, без передеплоя).
