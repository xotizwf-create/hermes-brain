...
- **Картинки**: `data[PARAMS][FILES][<id>][...]` (`type`/`name`/`urlDownload`) + `…[PARAMS][FILE_ID][N]`.
  ...
  OAuth-логин (403/HTML), скачать нельзя. Поэтому качаем серверно через REST `disk.file.get(id)` →
  `DOWNLOAD_URL` (`_refresh_bitrix_download_url`, тот же путь, что у daily-sync OCR).
  - **❗ Требует scope `disk` у вебхука `B24_TESTBOT_WEBHOOK_BASE`** (портал b24-0xrp3s). Без него
    `disk.file.get` → `401 insufficient_scope`, и картинка молча не читается (reply при этом работает).
    Добавляется в UI: Разработчикам → Входящий вебхук → галочка «Диск»; URL/токен не меняются.
...
### Документы (pdf/docx/xlsx/md/txt/csv) + ответ файлом PDF (2026-06-18)
Тот же путь, что у картинок, но для документов. В `_b24_message_extras` файл качается через
`disk.file.get → DOWNLOAD_URL` (нужен scope `disk` у вебхука), затем извлекается текст
...
«…». Извлечённое содержимое: …]` (бюджет ~12k симв/док). Зависимости добавлены в `requirements.txt`
(`pypdf`, `python-docx`, `openpyxl` — чистый python, без системных пакетов; venv на 186 уже обновлён —
если venv пересоберут, `pip install -r requirements.txt` вернёт их). Сообщение только с файлом (без
текста) больше не отбрасывается.

**Ответ файлом (PDF).** Системный промпт (`fmt` в `hermes_brain_answer`) учит агента: если просят
файл/PDF — сформировать ПОЛНОЕ содержимое в ответе и в конце поставить маркер `[[DELIVER_PDF: имя]]`.
Мост (`_b24_app_process`) снимает маркер (`_b24_extract_deliver`), рендерит ответ в PDF
(`_b24_text_to_pdf` — reportlab + кириллица DejaVu из `reportlab_font_paths`, поддержка `[b]…[/b]`,
списков `- `, заголовков `# `), сохраняет (`_b24_save_pdf_export`) и отдаёт **публичную токен-ссылку**
(переиспользует zoom-export маршрут: HMAC(filename+expiry) + TTL ~30 мин, `as_attachment`, mimetype по
расширению; абсолютный хост `mcp.m4s.ru`). Сканы PDF без текстового слоя не распознаются (нужен
OCR-рендер страниц — отложено). Коммиты Albery `559c546` (фича) + `35e23c7` (deps).
