...
## Гигиена индекса (2026-06-16)

Чистка на уровне чанкера (исходные документы НЕ трогаем, только поисковый индекс):
- **Исключаются артефакты `__sync_tmp__*`** — прерванный Drive-синк оставляет
  `__sync_tmp__<док>` дубли реальных документов (было 5 шт., ~76 КБ) → поиск отдавал документ
  дважды. Теперь не индексируются (`_is_excluded`); state пишется → инкрементальный прогон их
  пропускает. Корень проблемы — баг синка в `app.py` (оставляет temp-папки), артефакты копятся в
  `company_folders` до фикса синка (отдельная задача; можно и почистить 5 строк в БД).
- **Денойз листов** (`_normalize_for_index`): `<br>`→перенос, удаление `∅`, схлопывание пустых ячеек
  (`|  |  |`), удаление авто-заголовков пустых колонок (`Колонка 5:` — реальные колонки именованы:
  «Встречи:»/«Участники:»). После чистки чанки несут реальный текст таблиц, а не pipe-шум.

Итог: 42 дока → ~884 чанка по 33 документам (5 `__sync_tmp__` исключены). «Дубли «Копия …» НЕ трогаем
— это одиночные файлы с таким именем на Google Drive, а не дубликаты в БД.
