...
```text
версия:        v0.17.0 (обновлён 2026-07-06, пин на обкатанный коммит)
...
модель:        gpt-5.6-terra  (с 2026-07-12; было gpt-5.5), reasoning_effort: medium
дашборд:       http://127.0.0.1:9119   (hermes dashboard --tui)
```

### Модель агентов: gpt-5.6-terra (смена 2026-07-12)

**Что и когда.** 12.07.2026 модель ВСЕХ агентов Albery переведена `gpt-5.5` → **`gpt-5.6-terra`**
(`model.default` в `/root/.hermes/config.yaml`; пер-агентного override нет — правило владельца
«одна модель у всех»). GPT-5.6 вышла 09.07.2026 тремя тирами: Sol (флагман) / **Terra** / Luna.

**Почему — решение принято по A/B на РЕАЛЬНЫХ нагрузках, не по маркетингу:**

| нагрузка | gpt-5.5 | gpt-5.6-terra |
|---|---|---|
| задачи + инструменты | 32с | **22с** (сама добавила дедлайны и кликабельные ссылки) |
| поиск по регламентам | 61с | **31с** (те же документы, честное «регламента нет») |
| ТЯЖЁЛЫЙ ход (полная новостная сводка: огромный вход + длинный вывод) | 70с / 5.9k симв / 4 из 4 ключевых фактов | **46–47с / 2.4–2.6k симв / 4 из 4** (два повтора) |

⚠️ Методический урок: ПЕРВЫЙ тяжёлый прогон Terra пропустил 2 ключевые новости — одному сэмплу
верить нельзя, повтор ×2 дал полное покрытие. Не браковать (и не принимать) модель по одному ходу.

Выгода на подписке = не цена токенов (мы платим фиксированную подписку), а **меньший расход
лимита за ход** + скорость +50% + вдвое компактнее (владелец просил «короче, без воды»).

**Бэкапы и откат:**
- конфиг до смены: `/root/.hermes/config.yaml.bak-terra-20260712_215736`
- креды мозга: `/root/.hermes/auth.json.bak-20260712_220331` (600)
- откат: `cp /root/.hermes/config.yaml.bak-terra-20260712_215736 /root/.hermes/config.yaml &&
  systemctl restart hermes-gateway`

**Проверено после смены:** гейтвей поднялся чисто (3 патча применились), 6 кронов целы, живой ход
агента с инструментами 24с, мина `compression.codex_gpt55_autoraise` = false (к 5.6 не применяется).

**Нюансы модели:** голая `gpt-5.6` НЕ работает — нужен явный тир (`gpt-5.6-terra`). Проба модели:
`hermes -z "..." --provider openai-codex -m gpt-5.6-terra` (без `--provider` → «No LLM provider»).

### codex CLI ≠ мозг агентов (важно, разведано 2026-07-12)

`codex` CLI (npm `@openai/codex`, `/usr/bin/codex`) **НЕ участвует в ходах агентов**: на 186 он
вообще **не залогинен** («Not logged in»), а агенты работают — Hermes ходит к ChatGPT **своими**
токенами из `~/.hermes/auth.json` (провайдер `openai-codex` = OAuth-канал, не обёртка над CLI).
Поэтому версия CLI на модель агентов не влияет; он нужен только для **делегирования кодинга**.
12.07 обновлён профилактически `0.134.0 → 0.144.1` (версия, официально поддерживающая GPT-5.6);
после обновления живой ход агента — ок. Чтобы включить делегирование кодинга на 186, нужен
`codex login` (действие владельца) — сейчас эта возможность там недоступна.
